...
{
	protected $row_id,$quantity, $product_id, $price, $weight, $shipping, $title, $no_tax, $no_shipping, $meta, $item_options, $parent_item, $discounts;
	...
	{
		return $this->core->round( $this->tax() + $this->price() ) * $this->quantity();
	}
	...
	public function tax()
	{
		if (! $this->no_tax && $this->core->store->config('tax_plugin'))
		{
			if ($plugin = $this->core->store->plugin($this->core->store->config('tax_plugin')))
			{
				...
				{
					return $plugin->get_tax($this->price(), $this);
				}
			}
		}
		return 0; 
	...
	public function base_tax()
	{
		if (! $this->no_tax && $this->core->store->config('tax_plugin'))
		{
			if ($plugin = $this->core->store->plugin($this->core->store->config('tax_plugin')))
			{
				if (method_exists($plugin, 'get_tax'))
				{
					return $plugin->get_tax($this->base_price(), $this);
				}
			}
		}
		return 0; 
