	...
	public function save_customer_info()
	{
		...
		foreach (array_keys($this->cart->customer_info()) as $field)
		{
			if ($this->EE->input->post($field) !== FALSE)
			{
				...
				if ($this->store->config('save_member_data') && $field_id = $this->store->config('member_'.$field.'_field'))
				{
					if (is_numeric($field_id))
					{
						if ($this->EE->customer_model->load_profile_edit())
						{
							$d = $this->cart->customer_info($field);
							if ($d !== NULL)
							{
								$member_data['field_id_'.$field_id] = $d; 

								$member_data['field_ft_'.$field_id] = 'none';
							}							
						}
						else
						{
		...
		if (bool_string($this->cart->customer_info('use_billing_info')))
		{
			...
			foreach ($billing_fields as $field)
			{
				...
				if ($this->store->config('save_member_data') && $field_id = $this->store->config('member_shipping_'.$field.'_field'))
				{
					if (is_numeric($field_id))
					{
						if ($this->EE->customer_model->load_profile_edit())
		...
		if ($this->EE->session->userdata('member_id') && ($this->store->config('save_member_data') || $this->EE->input->post('save_member_data')))
		{
			$this->EE->load->model('member_model');
			
			if ( ! empty($member_data))
			{
				if ($profile_edit_channel_id = $this->EE->customer_model->load_profile_edit())
				{
					...
					$fields = $this->EE->cartthrob_field_model->get_fields_by_channel($profile_edit_channel_id);

					// the custom data will only save if SOME member data has gotten into the system. So at least one bit of customer data has to be in
					foreach ($fields as $field)
					{
						if (isset($custom_data[$field['field_name']]))
						{
							$member_data['field_id_'.$field['field_id']] = $custom_data[$field['field_name']];
							
							$member_data['field_ft_'.$field['field_id']] = 'none';
						}
						if (preg_match('/^profile_(.*)/', $field['field_name'], $match) && isset($custom_data[$match[1]]))
						{
							$field_data = $custom_data[$match[1]]; 
							if (is_array($field_data))
							{
								$field_data = implode('|', $field_data);
							}
							
							$member_data['field_id_'.$field['field_id']] = $field_data; 
							
							$member_data['field_ft_'.$field['field_id']] = 'none';
						}
					}
					
					$this->EE->cartthrob_entries_model->update_entry($this->EE->profile_model->get_profile_id($this->EE->session->userdata('member_id')), $member_data);
				}
				else
				{
			...
			if ( ! empty($member))
			{
				$this->EE->member_model->update_member($this->EE->session->userdata('member_id'), $member);
			}
		}
		...
		if ($channel_id = element('channel_id', $data))
		{
			...
			if ($item instanceof Cartthrob_item)
			{
				...
				if ($field_id && $field_type = $this->EE->cartthrob_field_model->get_field_type($field_id))
				{
					...
					{
						return $this->EE->api_channel_fields->apply('cartthrob_price', array($data['field_id_'.$field_id], $item));
					}
