...
class Cartthrob_payments
{
		...
		if ($has_subscription || $force_vault)
		{
			...
			if (isset($options['create_user']))
			{
				...
				if (element('create_group_id', $options)  && ! empty($member_id))
				{
					...
					$this->set_member_group($member_id, element('create_group_id', $options)); 
	...
	public function set_member_group($member_id, $group_id)
	{
		...
		$this->EE->member_model->update_member($member_id, array('group_id' => $group_id)); 
	}
	...
	public function activate_member($member_id, $group_id = NULL)
	{
		...
		if ($this->EE->cartthrob->store->config('checkout_registration_options') === 'auto-login' ||  
				($this->EE->config->item('req_mbr_activation') !== 'manual' && $this->EE->config->item('req_mbr_activation') !== 'email'))
		{
			$this->set_member_group( $member_id, $group_id); 
			$this->EE->cartthrob_members_model->login_member($member_id);
		}
	}
		...
		else
		{
			...
			if ($auth['authorized'])
			{
				...
				if ($this->EE->cartthrob->cart->order('create_user') && (! $this->EE->session->userdata('member_id') ||  $admin))
				{
					...
					{
						$this->activate_member($member_id, $group_id ); 
						$secure_forms = FALSE; // we have to set this to false, due to EE's use of session id in secure forms checking. 
					}
				...
				////////////// begin permissions /////////////////////////////
				// @NOTE it's possibel that permissions are doubled up in the subscriptions lib and here. We'll have to check
				$permissions = array(); 
