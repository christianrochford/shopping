...
class Cartthrob
{
	...
	public function add_to_cart_action()
	{
		...
		if ( ! isset($data['quantity']) || (isset($data['quantity']) && $data['quantity'] !== '0' && $data['quantity'] !== 0))
		{
			if ($item = $this->EE->cartthrob->cart->add_item($data))
			{
				...
				else if (isset($item_subscription_options['permissions']))
				{
					$item->set_meta('permissions', $item_subscription_options['permissions']);
				}
			
				if ($this->EE->input->post('plan_id'))
				{
					$item->set_meta('plan_id', $this->EE->input->post('plan_id'));
				}
				...
				//@TODO remove unencrypted subscription options, not secure
				if ( ! empty($item_subscription_options['subscription_enabled']) || (($this->EE->input->post('SUB') && bool_string($this->EE->encrypt->decode($this->EE->input->post('SUB')))) || ($this->EE->input->post('subscription') && bool_string($this->EE->input->post('subscription')))))
				{
					...
					foreach ($this->EE->subscription_model->option_keys() as $encoded_key => $key)
					{
						...
						else if (isset($item_subscription_options['subscription_'.$key]))
						{
							switch($key)
							{
								case 'end_date':
	...
	public function checkout_action()
	{
		...
		$checkout_options['create_user'] = bool_string($this->EE->input->post('create_user')); // NOT create_member_id. This is lookin in the post 
		...
		{
			$this->EE->session->cache['cartthrob']['member_id'] = $checkout_options['member_id'] = $_POST['member_id'];
		}
	...
	*/
	...
	/**
 	 ...
 	 * @author Jubair Saidi
 	 **/
	// @TODO need to move order manager model into CT itself. 
	/*
 	public function most_purchased()
	{
 	 	$data = array();
		
 	 	$this->EE->load->add_package_path(PATH_THIRD.'cartthrob_order_manager/');
	...
	*/
	...
	public function debug_info()
	{
		...
		$output .= '<h2 style="color:#000;">&nbsp;&nbsp;'.$this->EE->lang->line('cartthrob_profiler_data').'  </h2>';
