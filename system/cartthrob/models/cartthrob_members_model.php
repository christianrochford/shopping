...
class Cartthrob_members_model extends CI_Model
{
	...
	public function update_member($create_member_id, $customer_info)
	{
		...
		$this->load->model(array('member_model', 'customer_model'));
		...
		foreach (array_keys($this->cartthrob->cart->customer_info()) as $field)
		{
			...
			// saving the data.
			if ($field_id = $this->cartthrob->store->config('member_'.$orig_field.'_field'))
			{
		...
		foreach ($pe_fields as $pe_field)
		{
			$pe_field_name = element('field_name', $pe_field); 
		 	if (array_key_exists($pe_field_name, $custom_data))
		 	{
				$pe_field_id = element('field_id', $pe_field); 
				if (!isset($member_data['field_id_'.$pe_field_id ]))
				{
					$member_data['field_id_'. $pe_field_id] = $custom_data[$pe_field_name]; 
		...
		{
			if ($profile_edit_channel_id = $this->customer_model->load_profile_edit())
			{
				$this->load->model('cartthrob_entries_model');
				
				$member_data['channel_id'] = $profile_edit_channel_id;
				
				$this->cartthrob_entries_model->update_entry($this->profile_model->get_profile_id($create_member_id), $member_data);
			}
			else
			{
				$this->member_model->update_member_data($create_member_id, $member_data);
			}
		}
		
		if ( ! empty($member))
		{
			$this->member_model->update_member($create_member_id, $member);
		}
	}
	...
	public function login_member($member_id)
	{
		...
		*/ 
		...
		if ($this->config->item('secure_forms') === 'y' && $this->input->post('XID'))
		{
			if (version_compare(APP_VER, '2.5.5', '<'))
			{
				$this->db->insert('security_hashes', array('date' => time() - 60, 'ip_address' => $this->input->ip_address(), 'hash' => $this->input->post('XID')));
			}
			else
			{
				$this->db->insert('security_hashes', array('date' => time() - 60, 'session_id' => $this->session->userdata('session_id'), 'hash' => $this->input->post('XID')));
			}
		}
	...
	public function simulate_member($member_id, $callback, $args = NULL)
	{
		...
		{
			$cache[$key] = $this->EE->session->userdata[$key];
			
			$this->EE->session->userdata[$key] = $value;
		}
