...
class Subscription_model extends CI_Model
{
	...
	{
		$this->db->select('COUNT(DISTINCT(order_id)) AS subs_count', TRUE);
		$this->db->from('cartthrob_subscriptions'); 
		
		$data = $this->db->get()->row_array();
		
		return $data['subs_count'];
	}
	 ...
	 * @return CI_DB_result
	 */
	public function get_pending_subscriptions($limit = FALSE, $offset = FALSE)
	{
		if ($limit !== FALSE)
		{
			$this->db->limit($limit);
		...
		$this->db->select('cartthrob_subscriptions.*, cartthrob_vault.id AS vault_id, cartthrob_vault.*, cartthrob_subscriptions.id AS id');
	...
	private function pending_subscription_filter()
	{
		...
		$this->db->from('cartthrob_subscriptions')
			 ...
			 ->where('total_occurrences > used_total_occurrences')
